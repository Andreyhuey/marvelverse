...
const CharacterSeries = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      fetch(
        `https://gateway.marvel.com/v1/public/characters/${characterId}/series?&limit=25&orderBy=-modified&ts=1&apikey=${process.env.REACT_APP_API_KEY}&hash=${process.env.REACT_APP_HASH}`
      )
  ...
  const handleSearch = (event) => {
    ...
    setLoading(true);
    fetch(
      `https://gateway.marvel.com/v1/public/characters/${characterId}/series?titleStartsWith=${searchTerm}&limit=25&orderBy=-modified&ts=1&apikey=${process.env.REACT_APP_API_KEY}&hash=${process.env.REACT_APP_HASH}`
    )
      .then((response) => response.json())
      .then((data) => {
        console.log(data.data);
      ...
      <div className="container py-5">
        <form className="d-flex justify-content-center" onSubmit={handleSearch}>
          <input
            ...
            type="search"
            value={searchTerm}
            placeholder="e.g search for series"
                  ...
                  <div className="border border-warning card my-3 bg-dark">
                    <div className="p-2 my-3">
                      ...
                      <ul className="list-group list-group-flush d-flex justify-content-between">
                        ...
                        <li className="list-group-item bg-dark text-white d-flex justify-content-between">
                          Start Year: <b>{c.startYear}</b>
                        </li>
                        <li className="list-group-item bg-dark text-white d-flex justify-content-between">
                          End Year : <b>{c.endYear}</b>
                        </li>
